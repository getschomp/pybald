...
class Template(object):
    def __init__(self, app, config=None):
            ...
            if app.config.template_helpers:
                template_helpers.extend(app.config.template_helpers)

            # set the default filters to auto-html escape all content
            default_filters = ['h', 'unicode']
            if app.config.template_default_filters:
                default_filters = app.config.template_default_filters
            ...
            class TemplateEngine:
                ...
                def __init__(self, template_path=None, cache_path=None):
                    self.project_path = app.config.path
                    try:
                    ...
                    fs_test = app.config.template_filesystem_check or app.config.debug or False
                ...
                def __call__(self, template=None, data={}, format="html"):
                    ...
                    '''
                    template_data = dict(app.config.page_options.items() + data.items())
            ...
            setattr(app, 'render', TemplateEngine())
            ...
            setattr(app, 'engine', CompatibilityProxy(app.render))
